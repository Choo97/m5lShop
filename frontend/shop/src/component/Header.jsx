...
import "../App.css";
import "./Header.css";
import { FaSearch, FaCog } from "react-icons/fa"; // 돋보기 아이콘
        ...
        <Container fluid>
          ...
          <Row className="align-items-center mb-2 mb-3">
            ...
            <Col md={5} className="d-none d-md-block">
              <div className="position-relative w-100">
                ...
                <FaSearch
                  ...
                  onClick={() =>
                    keyword.trim() && navigate(`/products?keyword=${keyword}`)
                  }
                />
              </div>
            </Col>
            ...
            >
              ...
              {/* 로그인 상태이고, 관리자일 때만 버튼 표시 */}
              {user.isLogined && user.role === "ROLE_ADMIN" && (
                <div
                  className="icon-btn"
                  onClick={() => navigate("/admin")}
                  title="관리자 페이지"
                  style={{ color: "#dc3545" }} // 눈에 띄게 색상 변경
                >
                  <FaCog />{" "}
                  {/* 설정 아이콘 (import { FaCog } from 'react-icons/fa';) */}
                </div>
              )}
              {user.isLogined && (
                <div
                  ...
                  title="로그아웃"
                  style={{ marginLeft: "1.5rem", color: "#888" }}
                >
                  <FaSignOutAlt />
                </div>
              )}
              ...
              <Nav className="nav-bottom-row align-items-center">
                ...
                <NavItem>
                  <NavLink
                    ...
                    className="nav-link-custom fw-bold"
                  >
                    신상품
                  </NavLink>
                </NavItem>
                <NavItem>
                  <NavLink
                    onClick={() => navigate("/products?type=best")}
                    className="nav-link-custom fw-bold"
                  >
                    인기상품
                  </NavLink>
                ...
                <NavItem className="me-4">
                  <NavLink
                    ...
                    className="nav-link-custom fw-bold text-danger"
                  >
                    할인상품
                  </NavLink>
                </NavItem>
                    ...
                    >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownItem
                          header
                          onClick={() =>
                            navigate(`/products?category=${cat.code}`)
                          }
                        >
                          전체 {cat.label}
                        </DropdownItem>
                        ...
                        {cat.subItems.map((sub) => (
                          <DropdownItem
                            ...
                            onClick={() =>
                              navigate(
                                `/products?category=${cat.code}&sub=${sub.code}`
                              )
                            }
                          >
                            {sub.name}
